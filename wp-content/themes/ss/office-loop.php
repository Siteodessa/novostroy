...
$office_sqrt =  get_field('площадь_офиса') ;
$office_section =  get_field('секция__офиса_в_доме') ;
$office_state =  get_field('состояние_офиса') ;
$office_descr =  get_field('описание_офиса') ;
$office_gallery =  get_field('фото_офиса_планировка') ;
$office_block =  get_field('район_офиса') ;
$office_bld =  get_field('застройщик_офиса') ;
?>
...
<figure class="im">
			    ...
			    <?php if ($office_gallery) { ?> <?= $office_gallery ?> <?php }?>
          <!-- // в этом поле нужно вытащить первую фотку извлекать придется, потому что содержимое обернуто в <p>  -->
...
<ul class="app_main">
  <li class="name"></li>
  <li class="mini-data">
     <div class="rayon"><p>Район: </p><strong class="dom_block">
       <?php if ($office_block) { ?>
 <?= $office_block ?>
         <?php }?>

     </strong></div>
    <div class="zastr"><p>Застройщик: </p><strong class="prc">
      <?php if ($office_bld) { ?>
<?= $office_bld ?>
        <?php }?>
  ...
  <li class="description">
		    ...
		    <?php if ($office_sqrt) { ?>
    <div>  Площадь:      <?= $office_sqrt ?>    </div>
          <?php }?>
		    <?php if ($office_section) { ?>
      <div>     Секция:     <?= $office_section ?></div>
          <?php }?>
		    <?php if ($office_state) { ?>
      <div>     Состояние:     <?= $office_state ?></div>
          <?php }?>
		    <?php if ($office_descr) { ?>
          <div>  <?= $office_descr ?></div>
          <?php }?>
  ...
  <li class="link">
    <div class="app_features">
      <div class="commerce"><p>Другие коммерческие помещения</p>
